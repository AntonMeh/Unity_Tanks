...
namespace Tanks.Complete
{
    public class TankShooting : MonoBehaviour
    {
        ...
        private GameObject m_MinePrefab;            // The mine prefab to drop
        private int m_MineCount = 0;                // How many mines the player has left to drop
        ...
        void HumanUpdate()
        {
            ...
            {
                // If we have mines equipped, drop a mine instead of shooting a shell!
                if (m_MineCount > 0 && m_MinePrefab != null)
                {
                    DropMine();
                    m_Fired = true; // Prevents charging/firing the normal shell
        ...
        public void EquipMines(GameObject minePrefab, int count)
        {
            m_MinePrefab = minePrefab;
            m_MineCount = count;
        }

        private void DropMine()
        {
            // Calculate a drop position slightly behind the tank
            Vector3 dropPosition = transform.position - transform.forward * 2f + Vector3.up * 0.5f;
            Instantiate(m_MinePrefab, dropPosition, Quaternion.identity);

            // Play a drop sound (you can use m_FireClip or assign a specific one)
            m_ShootingAudio.clip = m_FireClip;
            m_ShootingAudio.Play();

            m_MineCount--;

            // If we run out of mines, tell PowerUpDetector we don't have an active powerup anymore
            if (m_MineCount <= 0)
            {
                PowerUpDetector powerUpDetector = GetComponent<PowerUpDetector>();
                if (powerUpDetector != null)
