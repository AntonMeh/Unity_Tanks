...
namespace Tanks.Complete
{
    public class PowerUpDetector : MonoBehaviour
    {
        ...
        // Gives the tank a specific number of mines to drop
        public void PowerUpMines(GameObject minePrefab, int count)
        {
            // Note: We don't necessarily set m_HasActivePowerUp = true if we want them to be able 
            // to pick up other powerups while holding mines, but for consistency we can set it.
            // But since dropping mines takes time, it's usually better to just give ammo.
            // Let's just equip it.
            m_TankShooting.EquipMines(minePrefab, count);
        }
    }
}
