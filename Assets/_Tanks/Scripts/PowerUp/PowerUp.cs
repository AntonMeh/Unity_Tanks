...
namespace Tanks.Complete
{
    ...
    {
        public enum PowerUpType { Speed, DamageReduction, ShootingBonus, Healing, Invincibility, DamageMultiplier, MineLayer }
        ...
        [Header("Mine Layer")]
        [Tooltip("The Mine prefab to drop.")]
        [SerializeField] private GameObject m_MinePrefab;
        [Tooltip("How many mines the tank gets.")]
        [SerializeField] private int m_MineCount = 3;
        ...
        private void OnTriggerEnter(Collider other)
        {
            if (other.gameObject.CompareTag("Tank"))
            {
                ...
                if (!m_PowerUpDetector.m_HasActivePowerUp)
                {
                    ...
                    // The power up gives the tank mines to drop
                    else if (m_PowerUpType == PowerUpType.MineLayer)
                        m_PowerUpDetector.PowerUpMines(m_MinePrefab, m_MineCount);
